...
import response from "./responses/responseAlias.json" with { type: "json" };

test("Abu Abbas search, match name and year", () => {
	const reqBody = { name: "muhammad zaydan", year: "1948", country: "canada" };
...
test("Abu Abbas search, match name but don't match year of the other result", () => {
	const reqBody = { name: "muhammad zaydan", year: "1978", country: "canada" };
	const result = searchHits(reqBody, response);
...
test("muhammad zayda search, match nothing", () => {
	const reqBody = { name: "muhammad zayda", year: "1978", country: "canada" };
	const result = searchHits(reqBody, response);
